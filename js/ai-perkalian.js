...
const AIMult = (() => {
    ...
    function evaluateMove(board, row, col, playerIdx, opponentIdx, profile) {
        ...
        const winCheck = checkWinVirtual(board, row, col, playerIdx, MULT_BOARD_SIZE, MULT_WIN_LENGTH);
        ...
        const opponentWin = checkWinVirtual(board, row, col, opponentIdx, MULT_BOARD_SIZE, MULT_WIN_LENGTH);
    ...
    function evaluateOpponentBestResponse(state, aiMove, profile) {
        ...
        for (let humanPos = 0; humanPos < MULT_COLS; humanPos++) {
            if (humanPos === humanCurrentPos) continue;
            const product = (humanPos + 1) * (aiNewPos + 1);

            const availableCells = getAvailableCellsForValue(simulatedBoard, product);
            for (const cell of availableCells) {
    ...
    function chooseInitialPlacement(firstPionCol) {
        const difficulty = GameMult.getDifficulty();
